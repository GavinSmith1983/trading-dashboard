...
class PricingEngine {
    ...
    rules;
    channels;
    constructor(config = types_1.DEFAULT_PRICING_CONFIG, rules = [], channels = []) {
        this.config = config;
     ...
     */
    calculatePrice(product, channelId = 'amazon') {
        ...
        if (belowMinimumMargin && !atFloorPrice) {
            warnings.push(`Margin (${proposedBreakdown.marginPercent.toFixed(1)}%) below minimum (${this.config.minimumMarginPercent}%)`);
        }
        return {
            sku: product.sku,
     ...
     */
    calculateCostBreakdown(product, price, channel) {
        const advertisingPercent = channel.includeAdvertisingInMargin
            ? channel.defaultAcosPercent || 0
            : 0;
        return (0, types_1.calculateCostBreakdown)(price, product.costPrice || 0, product.deliveryCost || 0, channel.commissionPercent, channel.fixedFee || 0, channel.paymentProcessingPercent || 0, advertisingPercent, channel.vatPercent, channel.pricesIncludeVat);
    }
    /**
     * Find the first rule that applies to this product
     */
    findApplicableRule(product, currentBreakdown) {
     ...
     */
    ruleMatches(rule, product, breakdown) {
        ...
        if (conditions.skuPatterns && conditions.skuPatterns.length > 0) {
            const matches = conditions.skuPatterns.some((pattern) => new RegExp(pattern).test(product.sku));
            if (!matches)
                return false;
        }
        // Margin conditions
        if (conditions.marginBelow !== undefined) {
            if (breakdown.marginPercent >= conditions.marginBelow)
                return false;
        }
        if (conditions.marginAbove !== undefined) {
            if (breakdown.marginPercent <= conditions.marginAbove)
                return false;
        }
        // Stock conditions
        if (conditions.stockBelow !== undefined) {
            if (product.stockLevel >= conditions.stockBelow)
                return false;
        }
        if (conditions.stockAbove !== undefined) {
            if (product.stockLevel <= conditions.stockAbove)
                return false;
        }
        // Sales velocity conditions
        if (conditions.salesVelocityBelow !== undefined) {
            if (product.salesLast7Days >= conditions.salesVelocityBelow)
                return false;
        }
        if (conditions.salesVelocityAbove !== undefined) {
            if (product.salesLast7Days <= conditions.salesVelocityAbove)
                return false;
        }
        // Price conditions
        if (conditions.priceBelow !== undefined) {
            if (product.currentPrice >= conditions.priceBelow)
                return false;
        }
        if (conditions.priceAbove !== undefined) {
            if (product.currentPrice <= conditions.priceAbove)
                return false;
        }
        return true;
    }
     ...
     */
    calculateFloorPrice(product, channel) {
     ...
     */
    generateProposals(products, batchId) {
        const proposals = [];
        for (const product of products) {
            const result = this.calculatePrice(product);
            // Only create proposal if price changed
            if (Math.abs(result.priceChange) < 0.01)
                continue;
            const proposal = {
                proposalId: (0, uuid_1.v4)(),
                sku: product.sku,
